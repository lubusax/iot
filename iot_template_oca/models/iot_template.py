...
class IotTemplate(models.Model):
    ...
    def _get_keys(self, serial):
        if self.parent_id:
            _logger.info(f"iot_template_oca//models//iot_template.py -_get_keys there is a self.parent_id: {self.parent_id}")
            keys = self.parent_id._get_keys(serial)
        else:
            _logger.info(f"iot_template_oca//models//iot_template.py -_get_keys there is NOT a self.parent_id: {self.parent_id}")
            keys = {"serial": serial}
        keys.update({key.key: key._generate_value() for key in self.key_ids})
        _logger.info(f"iot_template_oca//models//iot_template.py -_get_keys keys: {keys}")
        return keys

    def apply_template(self, device, keys):
        self.ensure_one()
        for element in self.input_ids:
            _logger.info(f"iot_template_oca//models//iot_template.py -- apply_template ")
            _logger.info(f"element:{element} - device: {device}- keys: {keys}")
...
class IotTemplateInput(models.Model):
    ...
    def _apply_template(self, device, keys):
        ...
        _logger.info(f"iot_template_oca//models//iot_template.py -- iot template input _apply_template ")
        _logger.info(f"real_vals: {real_vals}, real_vals.serial {real_vals['serial']},real_vals.passphrase {real_vals['passphrase']}")
